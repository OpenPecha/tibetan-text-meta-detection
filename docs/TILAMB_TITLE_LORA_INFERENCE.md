# TiLamb title LoRA — inference guide

Run inference with the fine-tuned title adapter:

**[ganga4364/tibetan-metadata-title-tilamb-lora-pilot](https://huggingface.co/ganga4364/tibetan-metadata-title-tilamb-lora-pilot)**
...
| **Instruction** | Fixed title-extraction prompt (same as SFT training) |
| **Chat template** | Llama-2 style via `tokenizer.apply_chat_template` |

The model returns **JSON only** with title span(s):

```json
{
  "spans": [
    {"text": "exact substring from input", "start": 120, "end": 138}
  ]
}
```
...
def predict_title(segment: str, max_new_tokens: int = 256) -> dict:
...
if __name__ == "__main__":
    ...
    print(json.dumps(predict_title(segment), ensure_ascii=False, indent=2))
```
...
python -m llm_sft.inference \
  --jsonl data/llm_sft_pilot_10pct/title/test.jsonl \
  --row 0 \
  --task title \
  --adapter ganga4364/tibetan-metadata-title-tilamb-lora-pilot \
...
### 4.2 Logged single-segment script

```bash
bash scripts/run_title_lora_inference.sh
# or
python scripts/run_title_lora_inference.py <segment_file> [base] [adapter]
```

### 4.3 Benchmark evaluator
...
python eval_benchmark_rows.py \
  ...
  --predictions logs/tilamb_lora_predictions.jsonl \
  --metrics-out logs/tilamb_lora_metrics.json \
  --resume
```
...
```bash
LORA_DIR=ganga4364/tibetan-metadata-title-tilamb-lora-pilot \
...
hf download ganga4364/tibetan-metadata-llm-sft \
  --include "title/test.jsonl" "title/test_meta.jsonl" \
...
| LoRA | r=16, α=32 |
| Train script | `scripts/run_llm_sft_pilot_train.sh` |
| Upload script | `scripts/push_lora_to_hf.sh title` |
...
- [benchmark/report/PILOT_TITLE_BENCHMARK_REPORT.md](../benchmark/report/PILOT_TITLE_BENCHMARK_REPORT.md) — evaluation results
...
- [hub/title_tilamb_lora_pilot_README.md](../hub/title_tilamb_lora_pilot_README.md) — Hugging Face model card source
